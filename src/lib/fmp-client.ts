...
export async function fetchFmpAnalystConsensus(symbol: string): Promise<FmpAnalystConsensus | null> {
  ...
  // Analyst count: prefer lastQuarterCount / lastMonthCount from price-target-summary
  // (buy/hold/sell distribution endpoints not available on this FMP tier)
  const summaryRows = [...records(summaryRaw), ...records(legacySummaryRaw)];
  const summaryRow = summaryRows[0] ?? null;
  const ratingCount =
    numberFrom(summaryRow, ["lastQuarterCount", "lastMonthCount", "allTimeCount"]) ??
    numberFrom(targetRow, ["numberOfAnalysts", "numberOfAnalyst", "analystCount", "ratingCount", "analysts"]);
